...
app.use(cors({
  origin: ['http://localhost:3000', 'https://dineinpro.vercel.app', 'http://dineinpro.vercel.app'],
  credentials: true,
}));
...
app.post('/api/orders', async (req, res) => {
  ...
  try {
    ...
    // **FIX**: The query now includes `categoryid` to uniquely identify the table.
    ...
    await client.query('COMMIT');
    // ✅ ADD THIS LOGIC
    // Announce the new KOT to the specific restaurant's room.
    const roomName = `restaurant-${parsedRestaurantId}`;
    const kotPayload = {
        message: `New KOT received for Bill No: ${billNo}`,
        restaurant_id: parsedRestaurantId, // This is crucial for filtering
        order_id: orderId,
        bill_no: billNo,
    };
    ...
    await client.query('ROLLBACK');
    console.error('❌ Error placing/updating order:', err);
...
app.get('/api/orders/by-table', async (req, res) => {
  ...
  try {
    ...
    // Get all the item details for that specific order
    const detailsResult = await pool.query(
      `SELECT * FROM order_details WHERE order_id = $1`,
      [order.id]
    );

    // Combine and send back the complete order details as a single object
    res.json({
      ...order,
...
app.put('/api/orders/:orderId/finalize', async (req, res) => {
  ...
  // This now captures the customer details sent from the frontend
  const { paymentMode, customername, customerno, email_id } = req.body;

  // It still requires a payment mode to proceed
  if (!paymentMode) {
  ...
  try {
    ...
    // The UPDATE query now includes fields to set the final customer details
    const orderUpdateResult = await pool.query(
      ...
      [paymentMode, orderId, customername, customerno, email_id]
    );
    
    if (orderUpdateResult.rowCount === 0) {
      throw new Error('Order not found.'); // Handles cases where the order doesn't exist
    }

    const { restaurantid, tablenumber, categoryid } = orderUpdateResult.rows[0];

    // If the order was associated with a table, its status is updated to 'available'
    if (tablenumber && categoryid) {
       await client.query(
    ...
    res.status(200).json({ message: 'Bill finalized and table is now available.' });
...
app.post('/api/staff', async (req, res) => {
  const {
    ...
    staffphoto, idcardphoto, password, monthly_salary // Added monthly_salary
  } = req.body;
  ...
  try {
    ...
    // Added monthly_salary to the INSERT query
    ...
    const userCredentialsQuery = `
      ...
      VALUES ($1, $2, $3, $4)
    `;
    await client.query(userCredentialsQuery, [restaurantid, email, hashedPassword, role]);
